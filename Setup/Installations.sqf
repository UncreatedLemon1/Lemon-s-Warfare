...
while {true} do {
	if (_ArtyCount < _artyTotal) then {
		...
		_findPos = [position centerPoint, 600, 5000, 3, 0, 0, 0, [], position Lmn_activeAO] call BIS_fnc_findSafePos;
	...
	if (_AACount < _AAtotal) then {
		...
		_arty = selectRandom _artyTypes;
		_findPos = [position centerPoint, 600, 10000, 3, 0, 0, 0, [], position Lmn_activeAO] call BIS_fnc_findSafePos;
		_arty = [_findPos, 0, _arty, east] call BIS_fnc_spawnVehicle;
		_grp = _arty select 2;
		_grp setCombatMode "RED";
		_AACount = _AACount + 1;
